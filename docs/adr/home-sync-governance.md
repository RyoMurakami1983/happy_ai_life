...
- `config.json` は全体を上書きせず、managed な enterprise/global guard entry だけを更新し、それ以外は user-owned として保持する
- managed surface は `managed-manifest.json` でも公開し、managed file / managed entry / user-owned surface の境界をレビューしやすくする
