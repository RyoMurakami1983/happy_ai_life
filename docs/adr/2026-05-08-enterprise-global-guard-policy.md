...
1. `$HOME/.copilot/config.json` に managed な enterprise/global guard entry を持たせる。
...
### C. Home Sync の managed entry を正式な global guard にする

採用。

既存の `sync-to-home.ps1` が `$HOME/.copilot/config.json` に managed な enterprise/global guard entry を追加する方向性を持っており、既存設計を活かせる。
