...
> **現行補足**: 2026-04 の `session-continuity-retirement` により、home sync は `$HOME/.copilot/.github/hooks` へ hooks を搬送しない。repo hooks は `sync-to-repo.ps1` の明示配布に寄せ、既定は `SafetyOnly` とする。
