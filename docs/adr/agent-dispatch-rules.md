...
- 入口は引き続き skill / built-in を優先し、coding 関連の前提整理は `grill-with-docs` を使う
- `tdd-coder` は `/fleet` または明示指名を基本とする
- 前提成果物（受け入れ条件、設計書または implementation handoff、実装契約）が不足する場合は差し戻す
...
- skill-first 構造を壊さずに、TDD 実装だけを局所的に加速する
