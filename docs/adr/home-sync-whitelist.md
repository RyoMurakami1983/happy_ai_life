...
## 補遺: Phase 7 — user-level safety hook を `config.json` に upsert

Copilot CLI 1.0.39 の実機 smoke で、user-level hook は `%USERPROFILE%\.copilot\config.json` の `hooks` 定義として動作することを確認した。`COPILOT_HOME` 指定時は `hooks/*.json` も動作したが、`config.json` は公式 help と実機結果の両方で確認でき、既存設定との merge 境界も明確である。

そのため、home sync は `%USERPROFILE%\.copilot\hooks\scripts\guard_pre_tool.ps1` を managed script として配布し、`config.json` の `env.HAPPY_AI_LIFE_HOOK_ID = "happy-ai-life-safety-guard"` を持つ `preToolUse` entry だけを upsert する。他の `config.json` 設定や user-owned hook entry は保持する。plugin hook は direct/local smoke が未通過のため primary safety としない。
