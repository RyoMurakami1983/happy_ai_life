...
理由は、この場所が user-level hook として解釈される場所ではなく、そのままでは hook として実行されない inert bootstrap asset だったためである。repo-scoped hooks は `sync-to-repo.ps1` の明示配布に寄せる。

## 補遺: Phase 7 — user-level safety hook の配布境界

Copilot CLI の hook 仕様は変化があり、実機では user-level hooks が `config.json` だけでなく `%USERPROFILE%\.copilot\hooks\*.json`（hook ファイル）としても解釈される環境がある。

そのため、home sync は次を行う:

- `%USERPROFILE%\.copilot\hooks\scripts\guard_pre_tool.ps1/.sh` を managed script として配布する
- `%USERPROFILE%\.copilot\hooks\safety-guard.json` を managed hook file として配布する（ここから上記 script を呼ぶ）
- 互換性のため `%USERPROFILE%\.copilot\config.json` にも `env.HAPPY_AI_LIFE_HOOK_ID = "happy-ai-life-safety-guard"` を持つ managed entry を upsert する（既存設定や user-owned hook entry は保持する）

また、session continuity hooks は標準運用から封印し、home sync では配布しない。
