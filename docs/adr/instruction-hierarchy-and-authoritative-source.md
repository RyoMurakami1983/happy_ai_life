...
Copilot CLI のカスタマイズは、以下の 6 レイヤーに分散している。
...
| MCP | `~/.copilot/mcp-config.json` | 外部ツール接続設定 |
...
| 外部ツール接続 | `~/.copilot/mcp-config.json` | 接続設定の唯一の正本 |
...
| .NET guardrail（該当 repo のみ） | MCP の接続 JSON 本文 |
| docs 更新条件 | |
| コミット・PR 方針 | |
