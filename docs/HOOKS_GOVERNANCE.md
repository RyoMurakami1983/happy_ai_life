...
home sync は `config.json` の managed entry、`hooks/scripts/guard_pre_tool.ps1`、`scripts/guard_policy.py` を配布し、この managed entry を正式な enterprise/global guard として扱う。managed entry は `preToolUse` と `permissionRequest` の両方に入り、thin wrapper から shared Python engine を呼び分ける。既定では `-ExecutionPolicy Bypass` を付けず、PowerShell 7 / Core host をそのまま使い、Windows PowerShell host では `pwsh` があればそちらを優先する。
...
- `guard_pre_tool.ps1` / `guard_pre_tool.sh` が有効な host: `python3 or python or py -3`
- `session-continuity.json` が存在し、session hook が有効な repo: `node`
- 有効な `sessionStart` hook が GitHub issue 取得を使う場合: `gh`

つまり thin wrapper 化後の safety guard は、現在の host で選ばれた wrapper から shared Python engine を起動できることを前提に `python3 or python or py -3` を要求する。一方で legacy opt-in として `session-continuity.json` を持つ repo では、`node` と必要に応じて `gh` を preflight 対象に含める。`gh` は標準運用の baseline ではなく、session-start hook が open issue 取得に GitHub CLI を使う場合だけ追加される。
