...
不確実性が高い技術選定、UI 導線、外部 API 連携は、`prototype` で短時間に検証します。prototype は本実装ではなく判断材料として扱います。
...
full test と type check は必要なときだけ手元または manual workflow で流します。
...
どこかで secret が見つかると処理が止まります。`gitleaks` まわりで困った場合は [トラブルシューティング](TROUBLESHOOTING.md) を参照してください。
