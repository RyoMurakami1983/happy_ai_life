...
特に repo-local skill（`.github/skills/**`、`.agents/skills/**`、`.claude/skills/**`）の `allowed-tools: "*"` や `*` を含む同等の全許可は、企業利用では既定で禁止する。

例外が必要な場合でも、少なくとも次を満たす明示承認が必要である。

1. **必要性の説明** — なぜ least privilege な個別列挙で代替できないかを書く。
2. **適用範囲の限定** — どの repo / skill / 利用者に適用するのか、owner と reviewer を明記する。
3. **出口条件の明示** — rollback 方法、見直し時期、恒久化しない理由を書く。

installed plugin / approved skill は curated distribution として別管理だが、repo-local skill に同じ基準を自動で緩和してよいわけではない。
