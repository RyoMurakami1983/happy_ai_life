...
`pre-commit` では staged diff を `gitleaks` で検査するため、ローカルでも `gitleaks` CLI を利用できる状態にしてください。
...
The local `pre-commit` hook reuses the same `.gitleaks.toml`, so allowlist tuning is shared between local commits and CI.
