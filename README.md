...
uv run app.py repo-secure-check C:\path\to\your-repo
uv run app.py repo-bootstrap C:\path\to\your-repo
uv run app.py repo-bootstrap C:\path\to\your-repo --apply
```
...
>
> `home` sync では `-Mirror` / `--mirror` は**互換オプション**です。  
...
>
> そのため home sync は、managed directory の中では template に無い項目を削除しますが、`docs/furikaeri/`、既存の `%USERPROFILE%\.copilot\` 配下にある runtime data、`mcp-config.json` のような user-owned file は削除しません。  
...
downstream repo の local safety valve を確認するときは `repo-secure-check.ps1` または `uv run app.py repo-secure-check ...` を使い、repo instructions / Copilot hooks / `.githooks` / `core.hooksPath` の不足があれば `repo-bootstrap` で dry-run から案内します。
