...
`uv run app.py home` を使える環境では、通常はこちらを既定導線にしてください。PowerShell 直叩きは home sync 済み環境での fallback / automation 用です。
...
>
> `repo-template/` と `.github/hooks/` は managed surface として template に無い項目を削除します。`skills/` と `agents/` の extra 項目は削除しません。`docs/furikaeri/`、既存の `%USERPROFILE%\.copilot\` 配下にある runtime data、`mcp-config.json` のような user-owned file も削除しません。  
...
- `home-template/.copilot/`: 個人環境向けテンプレート。home sync では `skills/`、`agents/`、`docs/furikaeri/`、`copilot-instructions.md`、`mcp-config.sample.json` に加え、`repo-template/`、`.github/hooks/`、bootstrap 用 scripts、`scripts/home_sync_planner.py` を HOME 配下へ配布する
