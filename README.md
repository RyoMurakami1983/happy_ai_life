...
この repo を clone 済みなら、**先に VS Code を完全に閉じてから** もう一度 `copilot plugin update` を試してください。まだ失敗する場合だけ、`uv run app.py plugin-repair --dry-run --no-interactive` で対象と backup 先を確認し、その後 `uv run app.py plugin-repair --yes --no-interactive` で復旧できます。
