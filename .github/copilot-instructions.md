...
- custom skill / agent の作成・改善・検証 → `copilot-authoring-beta` を使う。内部で `skill` と `create-agents` に分岐するため、直接そちらを起点にしない。
...
- 既存 skill の書き方・型・運用の相談 → `skill-shihan` を呼ぶ。`copilot-authoring-beta` の対象は作成・改善・検証の導線であって、個別 skill の相談窓口ではない。
- 言語/skill 領域の実務判断・型の提示 → `dotnet-shihan` / `python-shihan` / `typescript-shihan` を呼ぶ。
