...
### マルチリポ対応時の調査フロー
複数の関連リポが存在する場合、以下の順序で確認を進める:
1. **リポ特定**: ユーザー要件が「どのリポに対するものか / 複数リポにまたがるか」を明示的に確認する。「このリポで実装するのか、それともほかのリポか？」と聞く。
2. **制約確認**: 対象リポ（複数の場合は各リポ）の既存コードから参照すべき設計パターン・フォーマット・命名規約・依存関係を確認。特に「複数リポ間の共通約束」があれば把握する。
3. **パス曖昧性確認**: ファイルパスやディレクトリ指定が複数にマッチする場合、確認質問を優先し、推測で進めない。

## Skill ディスパッチ（必須）
- **初期チェック**: 調査の最初に「複数の関連リポか / 既存の設計パターンと依存関係を確認したか / パス指定は曖昧でないか」の 3 点を確認する（調査フローを参照）。
