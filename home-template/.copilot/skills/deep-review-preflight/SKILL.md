...
| すでに PR コメントが来ている | この skill ではなくレビュー応答へ進む | `gh-pr-review-response` |
...
DeepReview を通過したら、結果を短く残して `gh-pr-workflow` へ進みます。
...
- **`gh-pr-workflow`** — DeepReview 後の PR 作成とレビュー待機
- **`gh-pr-review-response`** — 実際の PR コメント対応
