...
license: Personal
compatibility: "security-review.agent.md, code-quality-review.agent.md, built-in /review or code-review agent"
---
...
| 状況 | 次にやること | 使うもの |
|---|---|---|
| ローカル差分の事前レビュー（品質） | Step 1 から開始 | `code-quality-review` agent |
| ローカル差分の事前レビュー（セキュリティ） | Step 1 から開始 | `security-review` agent |
| 認証・権限・migration 等の両面変更 | Step 1 から開始 | 両方の agent |
...
1. 変更内容に応じて `security-review` / `code-quality-review` agent を使い分ける
   - 品質・回帰・非破壊性が中心 → `code-quality-review`
   - 認証・入力検証・機密データが中心 → `security-review`
   - 認証変更 + ロジック変更など両面にまたがる → 両方
2. custom agent がない場合は built-in `/review` または `code-review` agent を使う
...
- **重い review を既定化しすぎない**: 変更規模に応じて custom agent と built-in review を使い分けます。
