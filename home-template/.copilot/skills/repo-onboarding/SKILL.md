...
4. 不足がある場合は onboarding を続ける前に bootstrap task として次を実行する。以下は **home sync 済みの `%USERPROFILE%\.copilot\scripts\`** から実行する前提とし、script の既定値で `%USERPROFILE%\.copilot` を `SourceRoot` として扱う。
   - `git init`
