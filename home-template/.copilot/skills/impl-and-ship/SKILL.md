...
description: >
  ...
  までの後半サイクルを 1 つの入口で進める。Use when: 計画が揃っていて実装サイクルを開始したいとき、
...
- `repo-secure-check` で local safety valve（repo instructions / Copilot hooks / `.githooks` / `core.hooksPath`）を確認する
...
今回の slice で何を done とみなすかを固定します。最低でも次を残します:
- 今回の対象振る舞い
- 非対象
- 主要な test 観点
