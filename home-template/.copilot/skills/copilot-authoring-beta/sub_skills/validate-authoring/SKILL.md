...
agent では `name` / filename 一致、`Use when:`、`原則` を含む必須 section、step 構造を確認します。`## プロセス` だけでなく `## 設計プロセス` や `## レビュープロセス` も許容し、既存 agent 群と同じ型を保ちます。これは prose の良し悪しではなく、利用者が agent を見つけて安全に使えるかの最低線です。
