...
オーケストレーター主導で、TDD の Red-Green-Refactor サイクルを回しながら実装を進めます。設計の揺れが出たら `design-workshop` に、順序や分割の揺れが出たら PLAN mode に戻ります。trust boundary が変更された時点では built-in review または自己レビューで中間チェックします。
