...
bootstrap checkpoint → target repo の `.github/instructions/` 配布確認
  ↓                    interactive app なら bootstrap checklist を確認
...
- target repo を触る flow では、実装前に `.github/instructions/` が配布済みかを確認する。
...
- **target repo の前提を飛ばさない**: pilot / downstream repo に入るときは、`.github/instructions/` と bootstrap 状態を確認してから実装に入ります。
