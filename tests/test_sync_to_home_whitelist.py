...
# PowerShell wrapper smoke test: runtime unavailable fail-closed is below (line ~845)
# Command variant tests moved to pure engine tests in test_guard_policy_engine.py

def test_guard_pre_tool_blocks_ai_git_commit_no_verify_smoke(tmp_path: Path) -> None:
    """Minimal PowerShell wrapper smoke test. Full variant coverage in test_guard_policy_engine.py."""
...
def test_guard_pre_tool_denies_when_python_runtime_is_unavailable(tmp_path: Path) -> None:
    ...
    script.write_text(script.read_text(encoding="utf-8").replace("WaitForExit(15000)", "WaitForExit(250)"), encoding="utf-8")
...
# PowerShell wrapper smoke test: permissionRequest response shape
def test_guard_permission_request_response_shape_smoke() -> None:
    """Smoke test for permissionRequest event response shape. Full command coverage in test_guard_policy_engine.py."""
    result = _invoke_guard_pre_tool(
        {"toolName": "powershell", "toolArgs": {"command": "git commit -n -m test"}},
        cwd=ROOT,
...
# Dangerous command variant tests moved to test_guard_policy_engine.py
# Only one smoke test for PowerShell wrapper remains

def test_guard_pre_tool_blocks_enterprise_dangerous_command_smoke() -> None:
    """Minimal PowerShell wrapper smoke test. Full command variant coverage in test_guard_policy_engine.py."""
    result = _invoke_guard_pre_tool(
...
# Bash guard command variant tests moved to test_bash_guard_policy.py
# Policy customization tests moved to test_guard_policy_engine.py
# Only minimal bash wrapper smoke test remains

def test_bash_guard_pre_tool_falls_open_for_invalid_json_payload() -> None:
    """Bash wrapper boundary smoke test for invalid JSON."""
    bash_path = _bash_executable()
...
# Fallback logic tests moved to test_guard_policy_engine.py
# Boundary smoke tests for wrapper-specific concerns remain below

# Protected path logic tests (kept - these are integration smoke tests for path resolution)
...
# Tool name customization tests moved to test_guard_policy_engine.py (pure engine tests)

# Maintenance mode tests (kept - these are integration tests for time/state handling)
