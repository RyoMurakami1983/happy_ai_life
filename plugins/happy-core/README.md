...
`sessionStart` / `sessionEnd` based repo-local context continuity is sealed as a default workflow. Use `furikaeri` for explicit daily reflection, with `/chronicle standup` when experimental mode is available and `/share file [PATH]` only when a raw session export is intentionally needed.
