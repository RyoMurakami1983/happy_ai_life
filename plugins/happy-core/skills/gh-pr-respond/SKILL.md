...
Option A の標準ルート: implementation -> `gh-pr-create` -> 実際のレビューシグナル待ち -> `gh-pr-respond` -> 人間のマージ判断/引き継ぎ。このスキルは強調したレビュー応答フェーズでのみ開始します。
...
- **`gh-pr-create`** — PR作成・Issue連携・シグナル駆動のレビュー待機（上流ワークフロー）
- **`session-issue-autopilot`** — セッション全体を包み、PR作成/待機を `gh-pr-create`、レビュー対応をこのスキルへ委譲する上位ラッパー
- **`git-commit`** — コミット形式と原子的コミット（Step 5から委譲）
- **`gh-issue-create`** — 保留レビュー項目のフォローアップIssue作成
...
6. **外科的対応** (基礎と型) — 修正は今回の指摘に直接効く最小差分へ絞る。「なぜ」：長い目で見た Happy、レビュー容易性、トークン消費の省エネを同時に守れるため
...
| 改善提案 | 差分に直接効き、長期的な Happy を下げないなら修正 | 中 | 「ガード句の使用を検討して」 |
...
- 今回の指摘に直接効かない整理や、認知負荷だけ増やすついで修正
```
...
最重要の問題から順に対応します。各修正は焦点を絞った原子的な変更にします。**まず「この変更は今回のコメントに直接効くか」を確認し、効かない追加修正は入れません。**
...
✅ **推奨**: 最小のコードで最大の効果を狙い、長期的な Happy を下げる横展開を避ける
❌ **非推奨**: すべてのレビュー修正を1つの大きなコミットにまとめる
❌ **非推奨**: 「ついでに直せるから」で無関係な整理や抽象化を混ぜる
...
リンター、テスト、バリデーションスクリプトを実行して修正が既存の品質を低下させないことを確認します。**PR レビュー対応では、まず変更範囲に対応する focused checks を既定**にし、差分とリスクに応じて full quality gate を追加してください。検証なしでプッシュしないでください。

#### 既定: focused checks

- レビュー修正が小さく、影響範囲が読みやすい場合は、**変更したファイル・その契約・直接の依存に対応する focused checks** を先に実行する
- 例: 対象テストファイル、touched files に対する lint / type check、skill validator、最小の再現テスト
- 目的は、レビューのイテレーションを重くしすぎず、差分に対する回帰を素早く確認すること

#### full quality gate を再実行すべき条件
...
focused checks または full gate が失敗した場合、Step 5 に進む前にリグレッションを修正してください。
...
すべてのコメントに対応し、**今回の差分に必要な checks が通ったうえで**プッシュが完了したら、元のレビュアーに再レビューを依頼します。実際の更新と結びつけ、変化があるときだけ通知します。
...
- レビュー修正では、まず変更範囲に対応する focused checks を回し、条件に当てはまるときだけ full quality gate を追加する
...
| 4 | focused checks を実行し、必要なら full gate も追加 | 差分に必要な確認が通過 |
