...
name: git-commit
description: >
  ユーザーがコミットを依頼したら、文脈がある場合は diff なしで判断し、分からない場合だけ diff を確認して atomic commit を提案・実行する。
---
...
ユーザーが「コミットして」「コミットを作って」などと言ったら、次の 3 ステップで進めます。

1. 文脈が十分に分かっているなら `git diff` なしで判断する。分からない場合だけ `git diff` を確認する。
2. 変更意図ごとに分割候補を決める。1 コミット = 1 変更意図を基本とする。
3. ユーザーに確認を取り、承認後に commit する。
