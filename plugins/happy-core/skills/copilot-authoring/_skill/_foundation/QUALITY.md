...
| C4 | `## こんなときに使う` 互換のセクションがある、または thin skill である | 起動後の relevance 判断は body か trigger-rich な description のどちらかに寄せればよい。 |
| C5 | 明示的な workflow / router / Core Loop セクションがある、または thin skill である | 薄い skill では本文そのものが手順であり、重い見出しを必須にしない。 |
...
| ID | Check | Why it matters |
| --- | --- | --- |
| R1 | 非 thin skill では「こんなときに使う」が 3-8 個の bullet で書かれている | 少なすぎると意図空間を取り逃がし、多すぎると焦点がぼける。 |
...
| R13 | skill がゴール、成功条件、確認手段のいずれかを示す | skill を topic 説明ではなく自律実行可能な型にする。 |
