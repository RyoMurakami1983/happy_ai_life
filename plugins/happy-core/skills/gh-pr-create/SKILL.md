...
標準ルート: implementation -> `gh-pr-create` -> 実際のレビューシグナル待ち -> `gh-pr-respond` -> 人間のマージ判断/引き継ぎ。
...
- **`gh-pr-respond`** - 実際のレビューシグナル到着後のコメント分類・修正・返信・再レビュー依頼
- **`git-commit`** - コミット形式と原子的コミット（Step 1から委譲）
- **`git-initial-setup`** - ブランチ保護の初期設定
- **`gh-issue-create`** - Issue作成とトリアージ
...
| レビュー対応 | レビューシグナル到着時に `gh-pr-respond` へ委譲する | 返信内容を確認し、承認で十分か判断する |
...
| 未コミットの変更あり | `git-commit` に委譲してコミット後、戻る |
| コミット済・未push | `git push -u origin BRANCH` してから Step 3 へ |
| push済・PR未作成 | Step 3（PR作成）へ進む |
| PR既存 | PRステータスとURLを報告 |

> **重要**: 未コミットの変更がある場合は `git-commit` ワークフローに委譲してください（先にコミット、その後戻る）。mainにいる場合は、コミット前に必ず feature branch を作成してください。
...
PRを開いたら、能動的なポーリングを止めます。具体的なシグナルが来たときだけ `gh-pr-respond` へ、シグナルごとに1回だけ受け渡します。
...
|---|---|---|---|
| 新しいレビューが送信された | いいえ | `gh-pr-respond` を開き、コメントを1回確認 | シグナル前の再確認 |
| 自分に review request が来た | いいえ | `gh-pr-respond` を開き、コメントを1回確認 | シグナル前の再確認 |
| ユーザーが新規レビュー活動を共有した | いいえ | 1回だけ確認してから `gh-pr-respond` へ委譲 | シグナル前の再確認 |
...
- `gh-pr-respond` で再レビュー依頼を出した後は、新しいレビューシグナルまたは人間の明示的なマージ判断が来るまで、この待機モードへ戻る
...
- [ ] `skills/**/SKILL.md` を変更した場合、PR作成前に skill validator を実行している
  - **このリポジトリから実行**: `uv run python plugins/happy-core/skills/copilot-authoring/_skill/_eval/scripts/validate_skill.py plugins/<plugin>/skills/<skill_id>/SKILL.md`
  - **Installed plugin から実行**: `uv run python $HOME/.copilot/skills/skill/_eval/scripts/validate_skill.py $HOME/.copilot/skills/<skill_id>/SKILL.md`
...
- [ ] 必要なら `git-commit` でコミット
- [ ] ブランチを origin に push
- [ ] `gh pr create` で PR 作成（日本語本文 + `Closes #N`）
- [ ] PR URL を記録したら、シグナル駆動のレビュー待機へ入る
- [ ] 実際のレビューシグナルが来たら `gh-pr-respond` へ委譲し、修正・返信・再レビュー依頼を行う
...
**Q: レビューやマージはこのスキルで扱う？**
A: PR作成、シグナル駆動のレビュー待機、残PRブランチ向けの確認済みマージ後同期まで扱います。標準ルートは implementation -> `gh-pr-create` -> レビューシグナル待ち -> `gh-pr-respond` -> 人間のマージ判断/引き継ぎ です。
