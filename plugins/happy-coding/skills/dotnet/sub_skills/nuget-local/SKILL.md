...
name: nuget-local
description: >
  こんなときに使う: ローカルで自作 NuGet の pack と consume を切り分けて案内したい
  とき。remote repository から clone し、release build で nupkg 化して local feed に
  置き、利用側 repository で参照追加と build 確認まで進めたいとき。
---
