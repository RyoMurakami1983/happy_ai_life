...
name: impl-and-ship
description: >
  実装契約を受け取り、bootstrap 確認・実装（TDD）・eval・review・furikaeri・PR・PR review
  までの後半サイクルを 1 つの入口で進める。multirepository 環境では parallel または sequential で実行。
  Use when: 実装契約が揃っていて実装サイクルを開始したいとき、
  fleet で並列実装するか判断したいとき、実装後の review・furikaeri・PR を一気に進めたいとき。
---

# impl-and-ship — 実装から出荷まで

実装契約を受け取り、bootstrap 確認から実装・eval gate・review・furikaeri・PR サイクルまでを 1 つの入口でつなぐ skill です。「fleet タイムアウト後の迷い（実装に入るべきか不明）」と「後半フローの責務不在」を解消するために存在します。

## こんなときに使う

このスキルは次のようなときに使います:
- grill-with-docs や design-workshop で実装契約が固まり、実装フェーズに入りたいとき
...
## ワークフロー: implementation contract から ship まで通す
...
- todo リストと受け入れ条件
- 技術的な前提条件とテスト方針の下地
- 戻り先の条件（FAIL → 実装修正、REPLAN_REQUIRED → grill-with-docs / design-workshop）

不足があれば `grill-with-docs` または `design-workshop` に戻ります。
...
- `FAIL` なら実装修正、`REPLAN_REQUIRED` なら grill-with-docs / design-workshop へ戻る条件
...
- ビルドエラーや前提逸脱が発生したら `grill-with-docs` / `design-workshop` に戻る
...
- `REPLAN_REQUIRED`: `grill-with-docs` または `design-workshop` に戻す
...
`gh-pr-create` を使い、feature branch → PR → レビュー待機 → 人間へのマージ引き継ぎを進めます。

### ステップ 9 — PR review 対応（gh-pr-respond）

PR にレビューコメントが届いたら `gh-pr-respond` を使い、分類・修正・返信・再レビュー依頼まで標準フローで進めます。

### ステップ 10 — furikaeri（gh-pr-respond 完了後）

PR review が一段落したら `furikaeri` で学びを整理します。PR review で得た気づきも含めて振り返ることで、最も充実した記録になります。
...
- **furikaeri は PR review 後**: PR review で得る学びが反映されるよう、原則として `gh-pr-respond` の後に行います。
