...
- `design-and-plan` の判断材料や `implement` の実装契約に、実測の根拠を足したいとき
...
- `interview-with-docs` — prototype 前に用語・前提を揃える
- `design-and-plan` — prototype の観測結果を設計判断へ反映する
- `implement` — 実装契約が固まった後、TDD で production slice を進める

## 注意点

- prototype を成果物扱いしない。成果物にするなら `implement` で production code として作り直す。
