...
7. **長期的な Happy を守る** - token 消費や複雑性だけ増やす修正は採らず、最小のコードで最大の効果を狙う
...
- 入力値の異常（型、範囲、エンコーディング）
- null / None / 未初期化
- 境界値（0, 1, 最大, 空コレクション）
- 時刻 / タイムゾーン / ロケール
- 並行実行 / 競合状態
- 設定差異（環境変数, feature flag, モード）
- 依存ライブラリの更新
- 外部 API / I/O の失敗・タイムアウト
- エラーハンドリングの握りつぶし
...
仮説を立て、**1 つずつ**検証します。複数の仮説を同時に変更すると、どの仮説が正しかったか判定できなくなります。
...
外科的対応を既定にし、**今回の failure に直接効く変更だけ**を入れます。長い目で見て Happy にならない複雑化、広すぎる探索、token だけ消費する横展開は避けます。
...
3. **調査ログ** — 検証した仮説と証拠の要約
4. **根本原因** — 所有境界と壊れた箇所
5. **修正内容** — 応急 or 恒久、変更ファイルとコマンド
6. **再発防止策** — テスト化、ガード、ドキュメント
7. **証拠パス** — before/after artifact の保存先
...
- **証拠が示していないついで修正を積む**: 外科的対応から外れ、長期的な Happy と説明可能性を損なう
