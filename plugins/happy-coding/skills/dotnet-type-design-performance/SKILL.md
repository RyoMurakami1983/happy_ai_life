...
- **`git-commit`** — パフォーマンス重要な変更をアトミックに分けて記録
...
✅ メソッドが同期的に返すことが多い場合（例：キャッシュヒット）、`ValueTask<T>`を使って`Task`のアロケーションを回避します。常に非同期のI/O操作には`Task<T>`を使用します。詳細は[応用パターン](references/advanced-performance-patterns.md)を参照。
...
**解決策**: ストリーミングには`IAsyncEnumerable<T>`、バッチ並列処理には`Task.WhenAll`を使用します。詳細は[応用パターン](references/advanced-performance-patterns.md)を参照。
...
- **応用パターン**: [ValueTask、Span、非同期列挙](references/advanced-performance-patterns.md)
