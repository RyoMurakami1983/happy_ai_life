...
各モジュールの設計書を作成する。対象技術で自然に実装できる形かを、オーケストレーターまたは repository instructions に沿って確認する。
...
- `plugins/happy-coding/skills/sdd/sub_skills/split_multi_repo_plan/SKILL.md` — multirepository での plan 分割（複数リポの設計を per-repo plan に分割するとき）
