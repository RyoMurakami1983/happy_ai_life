...
`docs/plan/NNN_PLAN.md` に phase、milestone、slice が書かれていても、それは説明と進捗可視化の区切りです。**明示的な blocker、HITL 判断、または `REPLAN_REQUIRED` がない限り、phase / slice ごとに止まらず次へ進みます。**
...
- blocker や明示的な確認待ちがない限り、完了した slice から次の slice / phase へ自走で進める
...
- `PASS`: 次の slice へ戻る（ステップ 3 から繰り返す。次の slice の contract を、対象振る舞い / 非対象 / test 観点 / test artifact path / test command / runtime launch command を含めて短く再固定する）
...
- handoff には最低でも、完了した slice / 残件 / 次に開く file または artifact / 再開 command または確認 command / 戻り先 skill を残す
...
- **次の slice では contract を短くフルで再固定する**: 差分だけの追記にせず、その slice の done / 非対象 / 証拠コマンドを見える形に戻します。
- **phase / slice は既定で自動停止点ではない**: blocker、HITL 判断、`REPLAN_REQUIRED` がない限り、次へ進めます。
