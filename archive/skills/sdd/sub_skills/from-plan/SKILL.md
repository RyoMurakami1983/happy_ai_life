...
compatibility: "implement"
---

# From Plan — Handoff to Implementation

このルートは `implement` skill へ直接 handoff するための sub-skill です。
ゴール駆動で使うため、最初に達成したいゴール、成功条件、確認手段を短く固定します。


計画（plan.md / todos / PLAN summary）が手元にある場合は、以下のステップで `implement` へ引き継いでください。
...
## ワークフロー: plan artifact から implement へ
...
### ステップ 2 — implement へ引き継ぐ

`implement` を呼び出し、以下の情報を渡してください：
```
...
`implement` が bootstrap 確認、contract checkpoint、実装（TDD）、eval gate、review、furikaeri、PR サイクル全体を担います。
...
- **multirepository 環境での並列実装**: plan.md に依存順序が明記されていれば、implement が `/fleet` で並列実装を判定します
